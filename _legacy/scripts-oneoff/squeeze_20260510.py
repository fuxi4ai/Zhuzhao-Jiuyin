#!/usr/bin/env python3
"""
四维度课件 · 行业逻辑 & 消息催化 终极榨取 v7
修复：处理"标题+内容在同一行"的格式（2026年起）
"""
...
START, END = '2025-10-19', '2026-03-30'

def parse_date(fn):
    m = re.match(r'(\d{2})(\d{2})(\d{2})', os.path.splitext(fn)[0])
    if m:
...
def extract(text):
    ...
    for pat in [
        ...
        if m:
            ...
            for stop in ['四、市场趋势','四、四维度','四、盘面','四、情绪面',
                         '五、关键时间','五、情绪第四','五、核心交易',
                         '六、市场情绪','六、财报解读','六、体系课']:
    ...
    for m in re.finditer(r'([^\n]{2,25}板块)的核心催化[是：:]\s*([^\n。]{10,200})', text):
        r['news_catalysts'].append(f'{m.group(1).strip()}核心催化: {m.group(2).strip()[:200]}')
    ...
    for m in re.finditer(r'([^\n]{2,20})价格.*?涨[至到达]\s*([\d.]+[^\n]{0,30})', text):
        sector = m.group(1).strip()[:15]
        price = m.group(2).strip()[:100]
        r['price_driver'].append(f'[{sector}] {price}')
...
def main():
    ...
    print(f'✅ 终极榨取 v7 完成！处理 {file_count} 个日期，{total_updates} 次写入\n')
    print('📊 提取数据量:')
    ...
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute(f'SELECT COUNT(*) FROM dim3_sentiment_tech WHERE date >= "{START}" AND date <= "{END}"')
    total = c.fetchone()[0]
    print(f'\n📊 交集范围内 dim3 填充率 (n={total}):')
    for col in ['emotion_stage','industry_logic','news_catalysts','policy_news','price_driver',
                'up_down_ratio','volume_trillion','limit_up','limit_down',
                'sentiment_description','trend_description','consecutive_boards']:
        c.execute(f'SELECT COUNT(*) FROM dim3_sentiment_tech WHERE date >= "{START}" AND date <= "{END}" AND {col} IS NOT NULL')
        filled = c.fetchone()[0]
        pct = filled/total*100
        bar = '█' * int(pct/2) + '░' * (50 - int(pct/2))
        print(f'  {bar} {col:25s} {filled}/{total} ({pct:.1f}%)')

    for col, label in [('industry_logic','行业逻辑'),('news_catalysts','消息催化'),
                        ('policy_news','政策新闻'),('price_driver','涨价驱动')]:
        print(f'\n📊 {label} 样本 (最新3条):')
        c.execute(f'SELECT date, {col} FROM dim3_sentiment_tech WHERE date >= "{START}" AND date <= "{END}" AND {col} IS NOT NULL AND LENGTH({col})>80 ORDER BY date DESC LIMIT 3')
