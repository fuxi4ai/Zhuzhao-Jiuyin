...
> **变体确认（2026-07-29 · 九儿课件班）**：同机制第二个受害者——`RAW_RECAP_DIR` 亦随根走：重定向后（本班 `/var/tmp/dbroot9`）先跑 `dedup_kejian.py scan`，返回 `total_files: 0`、**零报错零警告**，与真·今日无新课件**完全同貌**。与 resolver 案的关键差异：那次有 G030 绝对守卫响铃 abort，这次 scan 对「RAW_DIR 不存在」静默返回空清单（`if RAW_DIR.exists() else []`）——**差点误报「今日无新课件」收班**。靠此前 `ls` 过 Raw-Recap 有 242 个课件才起疑。软链（-001 预案）补上后复扫 242 正常。
> **判据**：重定向 ZZJY_DATABASE_ROOT 后首跑任何枚举/扫描，**必核 total 与挂载盘实数一致；total=0 一律先当路径错、后当无新增**。课件班标配从四件套扩为**五件套**（+`烛照九阴/Raw-Recap` 软链）。
> **改进建议（待哥哥批准，未动代码）**：`dedup_kejian.py scan` 对 RAW_DIR 不存在或空目录应 fail-loud（exit≠0 + ⚠ 提示），别静默返回空——G030 式绝对守卫的 scan 版。
...
## [GOTCHA-20260730-001] 小鲍课件（AI 撰写）会**捏造尚未发生的市场反应**——「事后验证式」表述一律不可当观测事实入库
**状态**: ✅ 已解决（口径已立，历史条目待抽查）
**优先级**: 🔴 高
**触发场景**: 260730 课件「盘面关键信号」段称『盘中发布的政治局会议通稿……但**在随后的交易日中**，相关板块并未出现预期的强势上涨，甚至部分科技股继续下探』，据此给出核心结论「政策利好钝化＝情绪极度脆弱」。独立核实后：该通稿由新华社 **2026-07-30 14:43 前后**发布（界面新闻转稿 14:43:34 / 科技日报 14:55:06 / 商务部 15:24），距 15:00 收盘仅约 17 分钟；课件当晚成文时 **7/31 尚未开盘**，所谓「随后的交易日」根本不存在。
**错误信息**: 无报错——课件读起来完全自洽，且该结论被写进「今日直播核心要点」，是最危险的假数形态（**结论级**污染，不是数值级）。
**解决方案**:
1. 该课件卷首自陈「以上内容是人工智能 AI 撰写」——**AI 二次加工层是独立于小鲍本人的新污染源**，P2 分级只覆盖「小鲍解读非一手源」，覆盖不了「AI 幻觉」。
2. 入库口径：凡课件出现 **事后验证式表述**（"在随后的交易日中""次日""后续""事实证明""市场随后……"）→ 一律标存疑、**不作为观测事实**，只保留为「课件主张」并注明可观察窗口。
3. 本次已按此订正 `industry_signals`(政策条) 与 `dim3_sentiment_tech.sentiment_description`：把结论限定到**真实可观察窗口＝尾盘约 17 分钟＋收盘集合竞价**，并在库内标注原文跨日表述错误 + 核实来源。
**预防措施**:
- **时点类断言必须外部核实，禁止靠课件内部互推**。本次九儿最初据 07-29 课件「政治局会议即将召开」外推出「7/29 盘后发布」，**日期与时段两处全错**——课件间的相对表述不能当时间锚。
- 同一份课件出现**自相矛盾**（本次：核心要点「盘后发布」vs 盘面段「盘中发布」）时，**不择一、不调和**，两处并录并标注，待外部信源裁决。
- 课件里的 `**` 屏蔽词（本次 4 处）**不猜词填空**，保留遮罩 + 标注词类；仅在哥哥确认后补全（本次「信仰**」→「信仰坍塌」由哥哥补）。
- 建议后续对 2026-06 以来的历史课件条目做一次**事后验证式表述抽查**（尚未执行）。

## [GOTCHA-20260730-002] 一列扛两个正交轴：`dim4_stock_analysis.confidence` 混装「判断把握度」与「信源分级」——且 DDL 注释成了帮凶
**状态**: ✅ 已解决（2026-07-30 拆列完成）
**优先级**: 🟡 中
**触发场景**: 该列同时出现 `高`/`中`（208+151）与 `P2`（99），任何按 confidence 的比较/筛选都在拿"判断强度"跟"信源等级"比大小。按 `source` 一切即现形：`小鲍老师` 350 行全是高/中、**零 P2**；`小鲍复盘课件` 109 行里 99 行是 P2 —— **两条写入路径各写各的轴，从没归一**。2026-07-23 最脏：课件源却打中/高，两轴撞在同一天。
**错误信息**: 无报错。混装静默存在约一个月（P2 自 2026-06-30 起被新脚本塞入）。
**根因**: 建表 DDL 注释写的是 `confidence TEXT, -- 置信度: 高/中/低`（判断轴），2026-06-30 起新 ingest 脚本按「P 分级」语义往同一列写 P2，**没人回头看注释**。注释与实际存值背离后，反而让后来排查者误以为"本该如此"。
**解决方案**: 拆两列（方案甲·全库口径统一优先）——`confidence` 归一为**信源轴**（P0/P1/P2，与 recap_daily / industry_signals 同义），新增 `conviction` 承接**判断轴**（高/中/低）。走**整表重建**而非 ALTER，目的就是把过时注释一并改正，不留"注释说 A、实存 B"的新陷阱。迁移：高208/中151 → conviction 原值；原 P2 那 99 行 conviction 留 NULL（**本就没记判断强度，不臆造**）；全表 confidence='P2'。下游 `populate_signal_targets.py` 改读 `conviction`（它喂的 `initial_confidence` 一直就是判断强度语义）。
**预防措施**:
- **任何列只承载一个语义轴**；新写入方若发现要写的值与列注释语域不符，**先改 schema，不要硬塞**。
- 整表重建时**注释与代码同批改**——SQLite 无法 ALTER 注释，这正是注释腐烂的温床。
- 跨表同名列必须同义：`confidence` 现已在 recap_daily / industry_signals / dim4_stock_analysis 三表统一为信源轴。
- ⚠️ 遗留：`stock_tracking.initial_confidence` 是更严重的**三轴混杂**（`高`/`中` + `P2` + `0.95` 这类浮点，另有 2855 行空串），尚未治理；目前全库无人读它，暂不阻塞。

> **写新 ingest 脚本者注意**：往 `dim4_stock_analysis` 插行时，`confidence` 一律填信源分级（小鲍课件=`P2`），判断把握度填 `conviction`（高/中/低）。别再往 confidence 里塞高/中。

---

## G033 — `intl_index_daily` 的 macro/futures 腿是「读数语义」，close 不是收盘价（消费侧陷阱）

**日期**: 2026-07-30 | **发现人**: CC（自身踩坑）

**问题**: 事件归因取 `intl_index_daily::US10Y` 的 07-30 值 4.66，据此写下「长端连续两天上行 +4bp」并进入 FOMC×伊朗案的判读。查 `updated_at` 发现该行写于 **2026-07-30 17:06:10 UTC ＝ 美东 13:06**，当时美股与美债**都还在盘中**——**4.66 是盘中快照，不是收盘价**，结论随即作废。

**根因**: **不是守卫失灵，是两套语义共存而消费方没看 kind。** `fetch_intl_index.py` 按腿分层（2026-07-28 界定）：

| kind | 腿 | `complete_only` | close 含义 |
|---|---|---|---|
| `overnight` / `us_stock` | NASDAQ·NVDA·AVGO·LITE·SPCX | **True** | 开盘中守卫，丢末根盘中快照 → **收盘价** |
| `futures` / `macro_rate` / `macro_commodity` | JP_FUT·US10Y·BRENT | **False** | 保留取数时点快照 → **读数，可能是盘中值** |

macro 腿服务的是日报五因 **F5「外部紧缩」**——那是方向性温度计，读数越新鲜越好，**刻意不开守卫**。所以同一次运行里美股个股腿正确地没写 07-30、US10Y 却写了，是**设计如此**。

**误判过程（值得记）**: CC 先把这现象判成「守卫对 ^TNX fail-open 的 bug」，并已拟出「移植 us_anchor 守卫 + 元数据不可判定即丢末根」的改动清单——**照此改会直接改掉 F5 的取数语义**。读到 L103 docstring 与 L229 `_complete = kind in (...)` 才发现是刻意分层，当场撤回。**险些用「归因场景的需求」去改一个不为归因服务的腿。**

**解决方案**（2026-07-30 · 零逻辑改动）:
- `INDICES` 表三条读数语义腿的 `note` 各加一句 **`⚠读数语义腿（无盘中守卫）：close 可能是盘中快照、非收盘价`**，US10Y/BRENT 另注明「归因/回测取收盘请用 H.15 或 FRED DGS10 / 官方结算价」。新写入的行自带该 note，历史行不回填。
- 表下方补「语义分层」注释块，写明**这是刻意设计、不是漏修**。
- `complete_only` 判定与 `_last_bar_incomplete` **一字未改**。

**预防措施**:
- **取一个数之前先问它承诺的是什么**。同一晚 CC 在同一条证据链上连栽三次：4.70 错在**日期**（实为 07-23 读数）、4.66 错在**时点**（盘中当收盘）、这次错在**语义**（读数腿当收盘腿）。三者共同点都不是算错，是**没核口径就用**。
- **一张表内允许多套语义，但必须让表自己说得出来**——kind 字段已经分了层，缺的是 note 没把「所以 close 是什么」讲明白。同 G032「任何列只承载一个语义轴」的姊妹面：**语义轴不同的行共存于一表时，行内必须自带语义标注。**
- **事件归因/回测需要真收盘的，一律另取官方源**（美债→H.15 / FRED DGS10；商品→官方结算价），不要图省事读 `intl_index_daily`。
- 判「这是 bug」之前先读设计注释与分支条件——**看起来漏修的地方，可能是别人替另一个消费方做过的取舍**。

**顺带核实（2026-07-30）**: 两表并存的标的只有 **LITE / NVDA / QQQ** 三个，在 `intl_index_daily` 里的 kind 均为 `us_stock`/`overnight`（**收盘语义**），与 `us_anchor_daily` 同语义 → **不存在「同一标的两表两套语义」的冲突**。`^TNX` 只在 `intl_index_daily` 单表存在。此结论限定了 G033 的影响面：只影响**跨表混用 macro 腿**的消费方，不影响两表交叉一致性检查（G030 那条另有其因）。

**★ 实测样本（2026-07-30 当日补数班首跑，同一 trade_date 前后两次取数直接对照）**——本条不再是理论风险，有量：

| 腿 | 盘中快照（写于美东 13:06） | 收盘后重取（次日 06:39 UTC） | 差 |
|---|---|---|---|
| `BRENT` | 89.64 | **89.03** | **−0.61（−0.68%）** |
| `JP_FUT` | 63545 | **63765** | **+220（+0.35%）** |
| `US10Y` | 4.66 | 4.66 | **0（恰好相等）** |

**读法**：① 风险是真的——BRENT/JP_FUT 的盘中值与收盘值确实不同，直接拿去做归因就会得出错的 Δ；② **US10Y 这次为 0 是巧合，不是保证**——当时正因为它「看起来没问题」，CC 才据 4.66 写下「长端连续两天 +4bp」的判读；**一个碰巧正确的数不能证明取数口径正确**。③ 由此再收一条判据：**验证口径要靠「重取对照」，不能靠「看这个数像不像对的」**。

**衍生待办 → ✅ 已复核结案（2026-07-31）· 本段质疑不成立，原归因成立**:

~~G030 把 US10Y 的「0.09pp 级差异」归因为「2dp 收盘价四舍五入」——但 2dp 四舍五入误差上限为 0.005，解释不了 0.09。该归因可能不成立，**真因或即本条的读数语义**。需单开一场复核 G030，不在本条顺手改（不动他人已结案条目）。~~

**① 编号订正**：该归因**不在 G030**（G030 是 `ticker_resolver` 种子源失联静默降级，与 US10Y 无关），而在 **G031「预防措施 ③」**——原文「`US10Y` 6 月起若干 0.09pp 级差异属 2dp 收盘价四舍五入，非本坑」。上面这段写 07-30 时引错了号，此后 `brain/TODO.md` 又照抄了这个错号。

**② 实测复核（`market_data.db::intl_index_daily` code=US10Y，20260601 起 42 个交易日，只读）**：逐日用表内 `close` 序列重算 `pct_chg` 并与存值对照 —— **差 >0.02pp 的共 31 天，超出 2dp 舍入传播上限的 0 天**。最大差 **0.2239pp @ 20260702**，当日上限 0.2242pp，压线在内。
